...
public class CardboardReticlePointer : MonoBehaviour
{
    ...
    /// </summary>
    private const float _RETICLE_MIN_DISTANCE = 0.1f;
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            {
                _gazedAtObject.GetComponent<ObjectController>()?.OnPointerExit();
                //_gazedAtObject.gameObject.GetComponent<ObjectController>()?.OnPointerExit(); - ошибка
                //_gazedAtObject?.SendMessage("OnPointerExit");
            }
        ...
        {
            OnPointerClick(); // ссылается на мое ниже
        }
